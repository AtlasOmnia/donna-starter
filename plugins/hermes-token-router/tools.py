    ...
    return {
        ...
        "parameters": {
            ...
            "properties": {
                "toolsets": {
                    "type": "array",
                    "items": {"type": "string", "enum": sorted(available_toolsets)},
                    "minItems": 1,
                    "uniqueItems": True,
                    "description": (
...
def _commit_candidate(
    ...
    *,
    ...
    represented = {
        owner_by_name[name]
        for name in names
        if owner_by_name.get(name)
    }
    base = list(base_enabled_toolsets or ()) if isinstance(base_enabled_toolsets, (list, tuple, set)) else []
    enabled: list[str] = [toolset for toolset in base if toolset in represented]
...
def select_and_commit_route(
    ...
    *,
    ...
    if RECOVERY_TOOL_NAME not in selected and (
        requested.intersection(available_toolsets) or (not requested and pins)
    ):
