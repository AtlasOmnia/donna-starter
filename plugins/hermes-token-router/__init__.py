...
def _ensure_host_admission(
    *,
    ...
    """Establish or reuse per-agent/session host admission exactly once."""
...
# Rev 6 adapter overrides.  Keeping the compatibility definitions above makes
# older imports stable while routing every live caller through the admission
# seam below.
...
def _route_tool_surface(
    ...
    **kwargs: Any,
    ...
    user_message = kwargs.get("user_message", "")
    turn_id = kwargs.get("turn_id") or getattr(agent, "_current_turn_id", "") or ""
    current_profile_cfg = _get_profile_config(cfg, profile_name=runtime_profile_name)
    if not current_profile_cfg.get("enabled", False):
